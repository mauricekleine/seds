...
function ProgramCard({ icon, title, description, href, color }: ProgramCardProps) {
  return (
    <Link
      to={href}
      className="group block bg-white border border-gray-200 rounded-lg p-6 hover:shadow-lg hover:border-green-500 transition-all duration-200"
    >
      <div className={`${color} mb-4`}>{icon}</div>
      <h3 className="font-display text-lg text-gray-800 mb-2 group-hover:text-green-600 transition-colors">
        {title}
      </h3>
      <p className="text-sm text-gray-600 m-0 leading-relaxed">{description}</p>
    </Link>
  );
}
